...
let
  ...
  mkFlakeFor = { ghc, cross, profiled }:
    let
      ...
      flake = pkgs.haskell-nix.haskellLib.mkFlake project {
        # NOTE: TODO comment we append the ghc & project to the shell so that we can retrieve 
        # them later when making the devShell.
        devShell = project.shell // { inherit ghc cross profiled project; }; # TODO oyeah
      };
    in
  ...
  addUserPerSystemOutputs = flake:
    let
      flake' = import flakeopts.perSystemOutputsFile {
        inherit inputs systemized-inputs flakeopts pkgs; 
        inherit (flake) projects; 
      };
    in
