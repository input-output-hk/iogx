{ unvalidated-config, l }:
let
  c = unvalidated-config;

  findFile = path: default:
    if l.hasAttrByPath path c then
      l.attrByPath path c
    else if l.pathExists (c.repoRoot + "/nix/${default}/default.nix") then
      import (c.repoRoot + "/nix/${default}/default.nix")
    else if l.pathExists (c.repoRoot + "/nix/${default}.nix") then
      import (c.repoRoot + "/nix/${default}.nix")
    else
      (_: { });
in
rec {
  ...
  devenvModule = findFile [ "devenvModule" ] "devenv-module";
  ...
  haskell = rec {
    ...
    projectModules = findFile [ "haskell" "projectModules" ] "haskell-project-modules";
